# Casper: Agent-Native Mac Automation with TypeScript, Deno & Rust

Casper is a TypeScript API for macOS desktop automation, backed by Rust FFI
and designed for LLM agent loops. Instead of flat function calls
(`click(x, y)`, `listWindows(pid)`), everything is an entity with methods:
`App`, `Window`, `Element`, `Keyboard`.
